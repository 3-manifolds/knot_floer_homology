...
void Simplify()
{   int x=INT_SIZE(GeneratorList), y=INT_SIZE(ArrowList); 
    if(y==0) return;
	...
	{int From=(Candidates[i]).second; //Trying to eliminate "From"
	 vector<int> X=Maps1[From]; if( X.size()==0 ) continue;
	   ...
	   {Arrow Arr=ArrowList[X[j]];  int c=Arr.EndingGen;
	        ...
	        && Arr.MonomialIndex == 0  && INT_SIZE(Maps2[c]) < Connectivity) 
	      {index=j; To=c; Connectivity=INT_SIZE(Maps2[c]);}
	   }
         if(index==-1) continue; 
	 ...
	 //at this point all the zig-zag arrows are added. 
         ...
         //rearrange the data: 
         if(CurrentSize > 2*OldSize + 10000 && i<INT_SIZE(Candidates)-1) 
    ...
    if(deleted>0) 
      {int Write=0;
       ...
       Write=0;
       for(int j=0; j<INT_SIZE(ArrowList); j++)
