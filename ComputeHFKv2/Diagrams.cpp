...
void MorseCode::Print(ostream & os) const {
  os<<"Morse Code: ";
  for (int i = 0; i < INT_SIZE(MorseList); i++)
    if (MorseList[i] >999)
      os<<"Max("<< MorseList[++i]<<"), ";
    else if (MorseList[i] >-1000)
      os<<MorseList[i]<<", ";
    else if (i < INT_SIZE(MorseList)-1)
      os<<"Min, ";   
    else 
      os<<"Min.\n";
  os<<"Girth: "<<Girth<<"\n\n";
    ...
    for(int T=0; T< R; T++)
	    ...
	    int aa=INT_SIZE(MostConnected);
	       ...
	       return MorseCode(Empty, -1);
             ...
             int t=0;
             while(t<INT_SIZE(temp) && temp[t] != V[0] && temp[t] != V[1] 
		 ...
		 for(int a=(int)FirstPosition; a>0;a--) 
	      ...
	      {for(int a=(int)FirstPosition; a>0;a--)
            ...
            if (INT_SIZE(temp)>B) B=INT_SIZE(temp); 
