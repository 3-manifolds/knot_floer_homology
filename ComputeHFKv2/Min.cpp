...
{   vector<Arrow> Z; 
    ...
    for(int Middle=0; Middle<GeneratorList.size(); Middle++)
...
{   int aa= MatchingList[0]; int bb= MatchingList[1];
    ...
    for(Arrow A: ArrowList)
      {monomial M=MonomialStore[A.MonomialIndex];
       ...
       if (x1 %4 ==2 and x2 %4==2 and a1==0 and  b1>0 ) MoveU2.push_back(A); //local weight: U2^b1           b1 >  0
       if (x1 %8 ==4 and x2 %8==4 and a1==0)                                 //local weight: U2^b1,          b1 >= 0
	  {M[aa-1] =M[aa-1]+b1; // b1>0 is a "curved" action, (U_2,C_1), (U_2 ^2,C_1,C_1), (U_2^ b1, C_1,...,C_1)
	   ...
	   idem I2 =(x2/4)-1; 
           ...
           if(!TooFar(I1,I2) and NonZero(I1,I2,m))   
    ...
    if(MoveL2.size() >0 and MoveU1.size() >0) PartialSequence=Join(MoveL2, MoveU1); 
    for(int Level=1; Level<200; Level++) //Level 1: (L2, U1, R2), Level 2, (L2, U1, U2, U1, R2). 
         {if (PartialSequence.size()==0) break;
	      ...
	      // we get this action from Join(Join(MoveL2, MoveU1), MoveR2)
              ...
              Bridge--; 
              if(!TooFar(I1,I2) and NonZero(I1,I2,m) ) NewArrowList.push_back(A);
              Bridge++;
	     }
    ...
    RemoveMod(ArrowList);
    int Write=0;
    for(int i=0; i<GeneratorList.size();i++ ) //Update GeneratorList
