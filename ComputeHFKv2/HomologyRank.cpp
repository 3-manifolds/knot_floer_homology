...
{   int x=(int)OldComplex.Generators.size(), y=(int)OldComplex.Differential.size(), deleted=0;
	...
	{int r=(Candidates[i]).second;
	 ...
	 for(int m=0; m<INT_SIZE(NewDifferentials); m++)
	   {ChainArrow Q=NewDifferentials[m]; int a=Q.StartingGen; int b=Q.EndingGen;                   	               
	     ...
	     int s=0; int f=INT_SIZE(Maps1[a]);
           ...
           vector<int> V1;  
           for(int i3=0; i3<INT_SIZE(Maps1[From]); i3++) V1.push_back(Maps1[From][i3]);
           for(int i3=0; i3<INT_SIZE(Maps1[To]);   i3++) V1.push_back(Maps1[To][i3]);
           for(int i3=0; i3<INT_SIZE(Maps2[From]); i3++) V1.push_back(Maps2[From][i3]);
           for(int i3=0; i3<INT_SIZE(Maps2[To]);   i3++) V1.push_back(Maps2[To][i3]);
  
           for(int i4=0; i4<INT_SIZE(V1); i4++) ArrList[V1[i4]].Coeff=0; //now there are "invisible"
           DeletedNames[To]=1;
           DeletedNames[From]=1;
           deleted=deleted+2; 
	      ...
	      i<INT_SIZE(Candidates)-1) // deleting those with Coeff=0.
		...
		{ChainArrow arrow1=ArrList[j]; 
		 if(arrow1.Coeff !=0) 
	      ...
	      //Then reorder the remaining part of the Candidate list:
              for(int j=i+1; j<INT_SIZE(Candidates); j++)
